...
public interface StateNameGenerator {
     ...
     */
    String getInitAutomatonStateName();
    ...
    /**
     * @param mergingStateNames имена состояний, которые объединяются в одно состояние
     * @return уникальное имя состояния, полученного в результате слияния состояний
     * из {@code mergingStateNames}. Имена предварительно упорядочиваются
     * лексикографически и объединяются с использованием разделителя {@code ";"}
     */
    default String generateForMergingStates(Set<String> mergingStateNames) {
        return mergingStateNames.stream()
                .sorted()
